...
public class SearchController {
	...
	public String searchByName(@RequestParam("searchQuery") String searchQuery,
		...
		map.addAttribute("photoList", searchService.basicSearchPhotos(searchQuery, principal));
	...
	public String submitAdvancedSearch(@ModelAttribute ("search") Search search,
		...
		map.addAttribute("emptyPhotoMsg", "label.emptysearchphoto");
		map.addAttribute("photoList", searchService.advancedSearchPhotos(search, page, principal));
		map.addAttribute("search", search);
		map.addAttribute("sortingSelections", SortTypes.getSortTypesAsStrings());
	...
	public String addPhotosAdvancedSearch(@ModelAttribute ("search") Search search,
			BindingResult result,
			@RequestParam(value = "page", defaultValue = "0", required = false) int page,
			Principal principal, ModelMap map) {
		logger.info("Getting more photos for advanced search with name " + search.getName());
		map.addAttribute("emptyPhotoMsg", "label.emptysearchphoto");
		map.addAttribute("photoList", searchService.advancedSearchPhotos(search, page, principal));
		map.addAttribute("search", search);
		map.addAttribute("sortingSelections", SortTypes.getSortTypesAsStrings());
		logger.info(""
				+ "Completed retrieving more photos for search with name " + search.getName());
		return "photos/_photo";
	}
	
	@InitBinder
	protected void initBinder(WebDataBinder binder) {
		binder.registerCustomEditor(Set.class, "tags", new TagAsStringPropertyEditor());
	}
}
